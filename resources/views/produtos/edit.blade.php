...
<div style="max-width: 600px; margin: 40px auto; font-family: 'Segoe UI', sans-serif;">
    ...
    <div style="background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); padding: 40px;">
        ...
        <h1 style="color: #2d3748; font-size: 24px; margin-bottom: 30px;">Editar Produto</h1>
        ...
        <form action="{{ route('produtos.update', $produto->id) }}" method="POST">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <textarea name="descricao" rows="4" required
                          style="width: 100%; padding: 10px 15px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 14px; box-sizing: border-box;">{{ $produto->descricao }}</textarea>
            </div>
...
<a href="{{ route('produtos.index') }}"
   style="background: #718096; color: white; padding: 10px 20px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-flex; align-items: center;">
     Voltar
</a>
                <button type="submit"
                        style="background: #d69e2e; color: white; padding: 10px 20px; border-radius: 8px; border: none; font-weight: bold; cursor: pointer;">
                    Atualizar Produto
                </button>
            </div>
        </form>
